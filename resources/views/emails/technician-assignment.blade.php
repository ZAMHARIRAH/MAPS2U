...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #d9e4f2;">
    <tr>
        <td style="background:#0f2b55;padding:20px 24px;color:#ffffff;">
            ...
            <p style="margin:6px 0 0;font-size:13px;opacity:0.9;">Technician assignment notification</p>
        </td>
    </tr>
    <tr>
        <td style="padding:24px;">
            <p style="margin:0 0 12px;font-size:14px;">Assalamualaikum {{ $recipientName }},</p>
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;margin:0 0 18px;">
                ...
                <tr>
                    <td style="padding:12px;border:1px solid #d9e4f2;background:#f8fbff;"><strong>Client</strong></td>
                    <td style="padding:12px;border:1px solid #d9e4f2;">{{ $clientRequest->full_name ?: $clientRequest->user?->name ?: '-' }}</td>
                </tr>
                <tr>
                    <td style="padding:12px;border:1px solid #d9e4f2;background:#f8fbff;"><strong>Location</strong></td>
                    <td style="padding:12px;border:1px solid #d9e4f2;">{{ $clientRequest->location?->name ?? '-' }}</td>
                </tr>
            </table>

            @if($ctaUrl && $ctaLabel)
                <p style="margin:0 0 24px;">
                    <a href="{{ $ctaUrl }}" style="display:inline-block;padding:12px 18px;background:#0f2b55;color:#ffffff;text-decoration:none;border-radius:10px;font-weight:600;">{{ $ctaLabel }}</a>
                </p>
            @endif
