...
<div class="page-header no-print"><div><h1>Report / Job Request</h1><p>Overview of submitted jobs with stronger management detail for client, task, workflow status, and timeline.</p></div><button class="btn ghost" onclick="window.print()">Print Report</button></div>
...
<section class="panel">
    ...
    <div class="report-table-shell"><table><thead><tr><th>Job ID</th><th>Client</th><th>Task Title</th><th>Location</th><th>Department</th><th>Status</th><th>Submitted</th><th>Completed</th><th>Cost</th></tr></thead><tbody>@forelse($items as $item)<tr><td>{{ $item->request_code }}</td><td>{{ $item->full_name }}</td><td>{{ $item->primaryTaskTitleName() ?? ($item->requestType?->name ?? '-') }}</td><td>{{ $item->location?->name ?? '-' }}</td><td>{{ $item->department?->name ?? '-' }}</td><td>{{ $item->adminWorkflowLabel() }}</td><td>{{ $item->created_at->copy()->timezone('Asia/Kuala_Lumpur')->format('d M Y H:i') }}</td><td>{{ $item->finance_completed_at ? $item->finance_completed_at->copy()->timezone('Asia/Kuala_Lumpur')->format('d M Y H:i') : '-' }}</td><td>RM {{ number_format(is_numeric(data_get($item->approvedQuotation(), 'amount')) ? (float) data_get($item->approvedQuotation(), 'amount') : 0, 2) }}</td></tr>@empty<tr><td colspan="9">No records found.</td></tr>@endforelse</tbody></table></div>
</section>
</div>
@endsection
