...
<div style="display:grid;gap:22px;">
    <div class="panel" style="margin:0;">
        ...
        <table class="table"><thead><tr><th>Name</th><th>Role</th><th>Email</th><th>Phone</th><th>Action</th></tr></thead><tbody>@forelse($clients as $client)<tr><td>{{ $client->name }}</td><td>{{ $client->roleLabel() }}</td><td>{{ $client->email }}</td><td>{{ $client->phone_number }}</td><td><a href="{{ route('admin.clients.show', $client) }}">View</a></td></tr>@empty<tr><td colspan="5">No client accounts found.</td></tr>@endforelse</tbody></table>
    </div>
    <div class="panel" style="margin:0;">
    ...
    <div class="panel" style="margin:0;">
        ...
        <table class="table"><thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Action</th></tr></thead><tbody>@forelse($technicians as $tech)<tr><td>{{ $tech->name }}</td><td>{{ $tech->email }}</td><td>{{ $tech->phone_number }}</td><td><a href="{{ route('admin.technicians.show', $tech) }}">View</a></td></tr>@empty<tr><td colspan="4">No staff accounts found.</td></tr>@endforelse</tbody></table>
    </div>
</div>
@endsection
